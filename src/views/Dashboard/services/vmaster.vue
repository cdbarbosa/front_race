<template>
  <main id="master">
    ...
    <h3 :class="{'__no-create': [2, 3, 4, 5].indexOf($store.getters.user.role_id) !== -1, '__no-edit': [5].indexOf($store.getters.user.role_id) !== -1, '__no-download': [5].indexOf($store.getters.user.role_id) !== -1}">
      Serviços
      <button v-if="[1].indexOf($store.getters.user.role_id) !== -1" id="showClientInfo" @click="isUserInfoOpen = !isUserInfoOpen">Dados do Cliente</button> <!--- If found, render -->
      ...
      <div v-if="serviceSelected && [1, 4, 5].indexOf($store.getters.user.role_id) !== -1" @mouseenter="restoreServiceSelected" id="edit" @click="serviceSelected ? (serviceSelected.lock ? null : isEditActive = true) : null" > <!--- If found, render -->
    ...
    <transition name="fade">
      <div class="userInfo" v-if="isUserInfoOpen">
        <b-button icon="close" @click="isUserInfoOpen = false">X</b-button>
